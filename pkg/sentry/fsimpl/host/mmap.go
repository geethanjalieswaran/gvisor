...
func (i *inodePlatformFile) IncRef(fr platform.FileRange) {
	i.fdRefsMu.Lock()
	fsutil.IncFrameRef(&i.fdRefs, fr)
	i.fdRefsMu.Unlock()
}
...
func (i *inodePlatformFile) DecRef(fr platform.FileRange) {
	i.fdRefsMu.Lock()
	fsutil.DecFrameRef(&i.fdRefs, fr)
	i.fdRefsMu.Unlock()
}
...
func (i *inode) Translate(ctx context.Context, required, optional memmap.MappableRange, at usermem.AccessType) ([]memmap.Translation, error) {
	mr := optional
	// TODO(gvisor.dev/issue/1476): Support limitHostFDTranslation.
	// This probably would involve refactoring to share maxFillRange from
	// fsimpl/gofer as a common utility.
